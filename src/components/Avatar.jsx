...
export function UploadableAvatar({ username, name, size = 42 }) {
  ...
  async function handleFile(e) {
    ...
    if (file.size > 200 * 1024) { alert("Max 200KB — use a small image or compressed GIF"); return }
    setUploading(true)
    try {
      ...
      await update(dbRef(db, "users/" + username), { pfp: base64 })
  ...
  return (
      ...
      <div style={{ width: size, height: size, borderRadius: r, overflow: "hidden", background: "var(--s3)", border: "1px solid var(--b1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.38, fontWeight: 700, textTransform: "uppercase", color: "var(--accent)", position: "relative" }}>
        {pfp ? <img src={pfp} alt={name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : (name?.[0]?.toUpperCase() || "?")}
        <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", opacity: uploading ? 1 : 0, transition: "opacity 0.2s", flexDirection: "column", gap: 2 }}
          onMouseEnter={e => e.currentTarget.style.opacity = "1"}
          {uploading ? <span style={{ fontFamily: "monospace", fontSize: 10, color: "#fff" }}>saving...</span> : <span style={{ fontSize: 18 }}>📷</span>}
        </div>
      </div>
    </div>
  )
