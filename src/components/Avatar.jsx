...
export function UploadableAvatar({ username, name, size = 42 }) {
  ...
  async function handleFile(e) {
    ...
    if (file.size > 200 * 1024) {
      alert('Max 200KB — use a smaller image')
      return
    }
    setUploading(true)
    try {
  ...
  return (
    <div style={{ position: 'relative', flexShrink: 0, cursor: 'pointer' }}
      ...
      <input ref={inputRef} type="file" accept="image/*,.gif"
      ...
      <div style={{ width: size, height: size, borderRadius: r, overflow: 'hidden', background: 'var(--s3)', border: '1px solid var(--b1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: size * 0.38, fontWeight: 700, textTransform: 'uppercase', color: 'var(--accent)', position: 'relative' }}>
        ...
        <div
          ...
          style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: uploading ? 1 : 0, transition: 'opacity 0.2s', flexDirection: 'column', gap: 2 }}>
          {uploading
            ? <span style={{ fontFamily: 'monospace', fontSize: 10, color: '#fff' }}>saving...</span>
